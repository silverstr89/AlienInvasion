...
import (
	"flag"
	"fmt"
	al "github.com/silverstr89/AlienInvasion/pkg/aliens"
	mv "github.com/silverstr89/AlienInvasion/pkg/move"
	ut "github.com/silverstr89/AlienInvasion/pkg/util"
	"log"
	"os"
)
...
func main() {
    ...
    Reads map data from file, and runs simulation.

    Usage: main [flags]
                -map for data file
                -aliens for number of aliens
    				`)

		flag.PrintDefaults()
	}

	mapFile := flag.String("map", "maps/sample_map", "file path with map")
	numAliens := flag.Int("aliens", 1, "deployed aliens")
	...
	// creating a map.
	cities, world, err := ut.GenerateMap(*mapFile)
	if err != nil {
		log.Fatal(err)
	}
	...
	cities, aliens, community := al.AliensCreated(cities, *numAliens, world, cityGroup, destroyedMap)
	if err != nil {
		log.Fatal(err)
	}
	...
	for i := 1; i <= lv; i++ {
		...
		cities, aliens, citiesWith1Alien, aliensDoneMoving = mv.AlienMove(cities, aliens, randSeq, world, community, cityGroup, destroyedMap, citiesWith1Alien, aliensDoneMoving) // time complexity of O(4*len(citiesWith1Alien))
