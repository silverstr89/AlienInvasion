...
func RemoveAlien(cities []st.City, aliens []st.Alien, r int, world map[string]int, community map[int]int, cityGroup map[int]int, destroyedMap map[string]int) ([]st.City, []st.Alien) {
	...
	var c *(st.City)
	c = &cities[r]
	cities, aliens = dt.AlienDestroy(cities, aliens, c, 0, community)

	cities[r].Aliens[1].IsDead = true
	c = &cities[r]
	cities, aliens = dt.AlienDestroy(cities, aliens, c, 1, community)

	temp := make([]int, 0)
	cities, aliens, temp = dt.CityDestroy(cities, aliens, r, world, cityGroup, destroyedMap, temp)

	return cities, aliens
}

func AliensCreated(cities []st.City, num int, world map[string]int, cityGroup map[int]int, destroyedMap map[string]int) ([]st.City, []st.Alien, map[int]int) {
	...
	for i := 0; i < num; i++ {
		...
		if len(cities[r].Aliens) == 2 {
			...
			cities, aliens = RemoveAlien(cities, aliens, r, world, community, cityGroup, destroyedMap)
			continue
		}
	}
