...
func TestDestroy(t *testing.T) /*([]st.City, []st.Alien, int, map[string]int,[]int, map[int]int, map[string]int, map[int]int ) */ {
	...
	cities2, aliens2 = AlienDestroy(cities2, aliens2, &cities2[world2["Foo"]-1], 0, community2)
	if len(cities2) != 2 {
	...
	cities2, aliens2 = AlienDestroy(cities2, aliens2, &cities2[world2["Foo"]-1], 1, community2)
	...
	r2 := world2["Foo"] - 1
	cities2, aliens2, citiesWith1Alien2 = CityDestroy(cities2, aliens2, r2, world2, cityGroup2, destroyedMap2, citiesWith1Alien2)
