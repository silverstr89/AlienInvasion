...
import (
	"fmt"
	dt "github.com/silverstr89/AlienInvasion/pkg/destroy"
	st "github.com/silverstr89/AlienInvasion/pkg/struct"
	"math/rand"
	"time"
)

func RemoveFromCityOneAlien(cities []st.City, aliens []st.Alien, oldCity *(st.City), cityGroup map[int]int, citiesWith1Alien []int) []int {
...
func AddOneAlien(cities []st.City, aliens []st.Alien, oldCity *(st.City), world map[string]int, cityGroup map[int]int, citiesWith1Alien []int) []int {
...
func RemoveSecondAlien(cities []st.City, aliens []st.Alien, newCity *(st.City), world map[string]int, community map[int]int, cityGroup map[int]int, destroyedMap map[string]int, citiesWith1Alien []int) ([]st.City, []st.Alien, []int) {

	// O(1) time complexity.
	newCity.IsDestroyed = true
	// delete first alien from the alien array of the city.
	newCity.Aliens[0].IsDead = true
	cities, aliens = dt.AlienDestroy(cities, aliens, &cities[world[newCity.Name]-1], 0, community) //  O(1) time complexity.

	// delete the second alien from the alien array of the city.
	newCity.Aliens[1].IsDead = true
	cities, aliens = dt.AlienDestroy(cities, aliens, &cities[world[newCity.Name]-1], 1, community)
	r := world[newCity.Name] - 1
	fmt.Printf("city with index %v, was destroyed\n", r)
	cities, aliens, citiesWith1Alien = dt.CityDestroy(cities, aliens, r, world, cityGroup, destroyedMap, citiesWith1Alien) // O(1) time complexity.
...
func AlienMove(cities []st.City, aliens []st.Alien, a []int, world map[string]int, community map[int]int, cityGroup map[int]int, destroyedMap map[string]int, citiesWith1Alien []int, aliensDoneMoving int) ([]st.City, []st.Alien, []int, int) {
	...
	for i := 0; i < len(citiesWith1Alien); i++ {
		...
		if p == 1 {
			...
			citiesWith1Alien = RemoveFromCityOneAlien(cities, aliens, oldCity, cityGroup, citiesWith1Alien)
			...
			if len(newCity.Aliens) == 2 {
				...
				citiesWith1Alien = RemoveFromCityOneAlien(cities, aliens, newCity, cityGroup, citiesWith1Alien)
				...
				cities, aliens, citiesWith1Alien = RemoveSecondAlien(cities, aliens, newCity, world, community, cityGroup, destroyedMap, citiesWith1Alien)
			...
			if alien.Moves >= 10000 {
				...
				citiesWith1Alien = AddOneAlien(cities, aliens, newCity, world, cityGroup, citiesWith1Alien)
				//fmt.Println("print the cities \n", cities)
			}
		}
	}
